 ...
 * {@code smartpool.rate-limit.enabled=true}（默认）时，自动创建限流器。
 *
 * @since 0.1.0
 */
@AutoConfiguration
@ConditionalOnClass(TokenBucketRateLimiter.class)
@ConditionalOnProperty(prefix = "smartpool.rate-limit", name = "enabled", havingValue = "true", matchIfMissing = true)
public class RateLimitAutoConfiguration {

    private static final Logger log = LoggerFactory.getLogger(RateLimitAutoConfiguration.class);

    @Bean
    public RateLimiterConfig rateLimiterConfig(SmartPoolProperties properties) {
        SmartPoolProperties.RateLimit props = properties.getRateLimit();
