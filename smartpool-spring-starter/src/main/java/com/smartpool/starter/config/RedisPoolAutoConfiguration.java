...
import com.smartpool.pool.redis.RedisConnectionFactory;
import com.smartpool.pool.redis.RedisConnectionPool;
import com.smartpool.pool.redis.RedisConnectionValidator;
import com.smartpool.pool.redis.RedisPoolConfig;
 ...
 * {@code smartpool.redis.enabled=true}（默认）时，自动创建 Redis 连接池。
 *
...
@ConditionalOnProperty(prefix = "smartpool.redis", name = "enabled", havingValue = "true", matchIfMissing = true)
public class RedisPoolAutoConfiguration {
    ...
    @Bean
    public RedisPoolConfig redisPoolConfig(SmartPoolProperties properties) {
        SmartPoolProperties.RedisPool props = properties.getRedis();
