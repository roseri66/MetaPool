 ...
 * {@code smartpool.lock.enabled=true}（默认）时，自动创建分布式锁。
 *
 * <p>锁的 Redis 连接信息从 {@code spring.data.redis.*} 标准配置读取。
 * 默认锁 key 为 {@code smartpool:default}，可通过 {@code smartpool.lock.key} 覆写。
 *
 * @since 0.1.0
 */
@AutoConfiguration
@ConditionalOnClass({SmartReentrantLock.class, RedisClient.class})
@ConditionalOnProperty(prefix = "smartpool.lock", name = "enabled", havingValue = "true", matchIfMissing = true)
public class LockAutoConfiguration {

    private static final Logger log = LoggerFactory.getLogger(LockAutoConfiguration.class);
    private static final String DEFAULT_LOCK_KEY = "smartpool:default";

    @Bean
    public LockConfig lockConfig(SmartPoolProperties properties) {
        SmartPoolProperties.Lock props = properties.getLock();
    ...
    public SmartReentrantLock smartReentrantLock(
            ...
            @Value("${smartpool.lock.key:" + DEFAULT_LOCK_KEY + "}") String lockKey) {
