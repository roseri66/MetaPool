...
import com.smartpool.pool.db.ConnectionFactory;
import com.smartpool.pool.db.DbConnectionPool;
import com.smartpool.pool.db.DbPoolConfig;
 ...
 * {@code smartpool.db.enabled=true}（默认）时，自动创建数据库连接池。
 *
...
@ConditionalOnProperty(prefix = "smartpool.db", name = "enabled", havingValue = "true", matchIfMissing = true)
public class DbPoolAutoConfiguration {
    ...
    @Bean
    public DbPoolConfig dbPoolConfig(SmartPoolProperties properties) {
        SmartPoolProperties.DbPool props = properties.getDb();
