 ...
 *
 * <h3>端口配置</h3>
 * <p>优先级：构造参数 → 系统属性 {@code smartpool.prometheus.port} → 默认 9100。
 *
 * @since 0.1.0
 */
public final class PrometheusHttpServer {
    ...
    /** 系统属性名，用于覆盖默认端口。 */
    public static final String PORT_PROPERTY = "smartpool.prometheus.port";
    ...
    public void start() throws IOException {
        ...
        ExecutorService executor = Executors.newSingleThreadExecutor(
                daemonThreadFactory("smartpool-prometheus-http"));
        server.setExecutor(executor);
        server.start();

        System.out.println("[SmartPool] Prometheus endpoint started on port " + port
