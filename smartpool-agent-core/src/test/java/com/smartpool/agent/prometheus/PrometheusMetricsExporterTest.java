...
class PrometheusMetricsExporterTest {
    ...
    class BasicFunctionalityTests {
        ...
        void shouldIncludePoolIdTagAfterRegistration() {
            ...
            assertTrue(result.contains("smartpool_thread_active"),
                    "应包含线程池 active gauge");
            assertTrue(result.contains("smartpool_thread_execute_count_total"),
    ...
    class NamingConventionTests {
        ...
        void shouldAppendTotalSuffixForCounters() {
            ...
            assertTrue(result.contains("smartpool_thread_execute_count_total"),
                    "驼峰命名应转为蛇形 + _total: smartpool_thread_execute_count_total");
        }
        ...
        void shouldUseLowerCaseUnderscoreNames() {
            ...
            Pattern metricLinePattern = Pattern.compile("^smartpool_[a-z_]+\\{", Pattern.MULTILINE);
            // 提取所有 smartpool 开头的指标行
            for (String line : result.split("\n")) {
                if (line.startsWith("smartpool_") && !line.startsWith("#")) {
    ...
    class LabelWhitelistTests {
        ...
        void shouldOnlyHavePoolIdAsLabel() {
            ...
            // 移除注释行，检查实际指标行
            for (String line : result.split("\n")) {
                if (line.startsWith("smartpool_") && line.contains("{")) {
                    // 提取标签部分
    ...
    class All28MetricsTests {
        ...
        void shouldRegisterAllExpectedMetrics() {
            ...
            // 验证各池的前缀都出现
            assertTrue(result.contains("smartpool_thread_"), "缺少线程池指标");
            assertTrue(result.contains("smartpool_db_"), "缺少 DB 连接池指标");
            assertTrue(result.contains("smartpool_redis_"), "缺少 Redis 连接池指标");
            assertTrue(result.contains("smartpool_rate_limit_"), "缺少限流器指标");
            assertTrue(result.contains("smartpool_lock_"), "缺少分布式锁指标");
            assertTrue(result.contains("smartpool_memory_"), "缺少内存池指标");
