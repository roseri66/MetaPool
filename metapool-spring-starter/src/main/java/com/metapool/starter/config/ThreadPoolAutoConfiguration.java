...
import com.metapool.pool.thread.RejectedPolicyEnum;
 ...
 * {@code metapool.thread.enabled=true}（默认）时，自动创建线程池实例。
 *
 * @since 0.1.0
 */
@AutoConfiguration
@ConditionalOnClass(ThreadResourcePool.class)
@ConditionalOnProperty(prefix = "metapool.thread", name = "enabled", havingValue = "true", matchIfMissing = true)
public class ThreadPoolAutoConfiguration {

    @Bean
    public ThreadPoolConfig threadPoolConfig(MetaPoolProperties properties) {
        MetaPoolProperties.ThreadPool props = properties.getThread();
