 ...
 * {@code metapool.memory.enabled=true}（默认）时，自动创建内存池。
 *
 * @since 0.1.0
 */
@AutoConfiguration
@ConditionalOnClass(MemoryPool.class)
@ConditionalOnProperty(prefix = "metapool.memory", name = "enabled", havingValue = "true", matchIfMissing = true)
public class MemoryPoolAutoConfiguration {
    ...
    @Bean
    public MemoryPoolConfig memoryPoolConfig(MetaPoolProperties properties) {
        MetaPoolProperties.MemoryPool props = properties.getMemory();
