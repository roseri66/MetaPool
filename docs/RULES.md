...
2. 合入前 `mvn clean test` 必须全绿。当前基线（2026-08-13 实测 `mvn -B clean verify` 汇总；**不含 `smoke-consumer`**，它单独跑）：
   - **有 Docker：193 通过 + 0 跳过**
   - **无 Docker：178 通过 + 15 跳过**
...
### P-23 一个资源的 `health()` 抛异常，会击穿整个聚合健康
- **现象**：`DefaultResourceManager.health()` 直接把某个资源抛出的异常传播出去，
  于是 `/actuator/health` 整个报错 —— 其余资源是否健康完全看不到。
- **原因**：聚合循环里直接 `HealthStatus h = r.health();`，没有任何隔离。
  契约确实要求 `health()` 只返回瞬时快照、不抛异常，但**契约管不住第三方 adapter**，
  而 2.1 之后 SPI 扩展是明确鼓励的（`metapool.resources.<type>` 就是为它开的口子）。
- **为什么这条比看起来严重**：聚合健康正是**出事的时候**要看的东西。
  一个行为不端的适配器能让它在最需要的时刻整个不可用，而且运维看到的是一个
  **与真实故障无关**的异常，排查方向从一开始就被带偏。
- **修法**：逐个资源 try/catch，抛异常或返回 `null` 的一律**计为 DOWN 并在 detail 里点名**，
  其余资源照常参与聚合。**防复发**：`health_oneResourceThrowing_doesNotBreakTheAggregate`，
  已验证修复前该用例确实以 `IllegalState boom-health:rogue` 失败。
- **一条刻意的不对称**（同轮想清楚并写成测试钉住）：`close()` 容错（一个资源停机失败，
  其余照停），`health()` 容错（如上），但 **`bindMetrics()` 维持快速失败** ——
  绑不上指标属于接线错误、发生在启动期，**大声失败好过静默少一半指标**：
  后者会让人在故障时对着一块缺了曲线的看板做判断。判据是「这个失败发生在什么时候、
  瞒着人会怎样」，不是「统一都 try-catch 显得整齐」。
