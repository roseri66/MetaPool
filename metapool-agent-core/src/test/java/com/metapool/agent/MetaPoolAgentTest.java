...
import com.metapool.pool.thread.SmartThreadPoolExecutor;
...
class MetaPoolAgentTest {
    ...
    class AgentEntryTests {
        ...
        void shouldNotThrowOnPremain() {
            assertDoesNotThrow(() -> {
                Class<?> agentClass = Class.forName("com.metapool.agent.MetaPoolAgent");
                assertNotNull(agentClass);
            });
        }

        @Test
        @DisplayName("MetaPoolAgent 应为工具类（私有构造器）")
        void shouldBeUtilityClass() throws Exception {
            java.lang.reflect.Constructor<?> ctor = MetaPoolAgent.class.getDeclaredConstructor();
            assertTrue(java.lang.reflect.Modifier.isPrivate(ctor.getModifiers()),
                    "MetaPoolAgent 构造器应为 private");
        }
    }
