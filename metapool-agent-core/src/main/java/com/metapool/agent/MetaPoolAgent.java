...
import com.metapool.agent.interceptor.*;
import com.metapool.agent.prometheus.PrometheusHttpServer;
import com.metapool.agent.prometheus.PrometheusMetricsExporter;
...
/**
 * MetaPool Java Agent 入口。
 *
 * <p>通过 {@code -javaagent:metapool-agent-core.jar} 挂载，使用 ByteBuddy
 ...
 * <h3>使用方式</h3>
 * <pre>{@code
 * java -javaagent:metapool-agent-core.jar -jar myapp.jar
 * }</pre>
 *
 * @since 0.1.0
 */
public class MetaPoolAgent {
    ...
    private static AgentBuilder buildAgent() {
        return new AgentBuilder.Default()
                ...
                .ignore(nameStartsWith("com.metapool.agent.")
                        .or(nameStartsWith("net.bytebuddy.")))
                // --- 线程池 ---
                .type(named("com.metapool.pool.thread.SmartThreadPoolExecutor"))
                ...
                .type(named("com.metapool.common.pool.AbstractResourcePool"))
                .transform((builder, type, classLoader, module, pd) ->
                ...
                // --- 限流器 ---
                .type(named("com.metapool.pool.rate.limit.TokenBucketRateLimiter"))
                .transform((builder, type, classLoader, module, pd) ->
                        builder.visit(Advice.to(RateLimiterInterceptor.class)
                                .on(named("tryAcquire").and(takesArguments(0)))))
                // --- 分布式锁 ---
                .type(named("com.metapool.pool.lock.SmartReentrantLock"))
                .transform((builder, type, classLoader, module, pd) ->
                ...
                .type(named("com.metapool.pool.memory.MemoryPool"))
                .transform((builder, type, classLoader, module, pd) ->
    ...
    private static void startPrometheusEndpoint(String agentArgs) {
        try {
            ...
            System.err.println("[MetaPool] Failed to start Prometheus endpoint: " + e.getMessage());
        }
    }
